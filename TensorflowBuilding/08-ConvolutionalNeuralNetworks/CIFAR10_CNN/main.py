...
def build_second_net():
    ...
    conv1_1 = convolutional_layer(x, shape=[3, 3, 3, C1])
    conv1_2 = convolutional_layer(conv1_1, shape=[3, 3, C1, C1])
    conv1_3 = convolutional_layer(conv1_2, shape=[3, 3, C1, C1])
    conv1_pool = max_pool_2x2(conv1_3)
    conv1_drop = tf.nn.dropout(conv1_pool, keep_prob=keep_prob)

    conv2_1 = convolutional_layer(conv1_drop, shape=[3, 3, C1, C2])
    conv2_2 = convolutional_layer(conv2_1, shape=[3, 3, C2, C2])
    conv2_3 = convolutional_layer(conv2_2, shape=[3, 3, C2, C2])
    conv2_pool = max_pool_2x2(conv2_3)
    conv2_drop = tf.nn.dropout(conv2_pool, keep_prob=keep_prob)

    conv3_1 = convolutional_layer(conv2_drop, shape=[3, 3, C2, C3])
    conv3_2 = convolutional_layer(conv3_1, shape=[3, 3, C3, C3])
    conv3_3 = convolutional_layer(conv3_2, shape=[3, 3, C3, C3])
...
def run_simple_net():
    ...
    conv1 = convolutional_layer(x, shape=[5, 5, 3, 32])
    conv1_pool = max_pool_2x2(conv1)

    conv2 = convolutional_layer(conv1_pool, shape=[5, 5, 32, 64])
    conv2_pool = max_pool_2x2(conv2)

    conv3 = convolutional_layer(conv2_pool, shape=[5, 5, 64, 128])
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        for i in range(STEPS):
            print('i {} in range({})'.format(i, STEPS))
